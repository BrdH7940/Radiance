  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-xs font-medium text-slate-600">Plain text</span>
      </div>
      ...
      <div className="flex-1 flex flex-col min-h-[320px] rounded-[2.5rem] border border-white/10 bg-[#0a0f18] overflow-hidden transition-all duration-500 focus-within:border-indigo-500/40 focus-within:shadow-[0_0_0_1px_rgba(99,102,241,0.15)]">
        ...
        <div className="flex items-center gap-2 px-5 py-3 border-b border-white/5">
          <FileSearch className="w-4 h-4 text-slate-500" strokeWidth={1.5} />
        ...
        <textarea
          ...
          className="
            flex-1 w-full px-5 py-4 bg-transparent text-slate-300 text-sm
        ...
        <div className="flex items-center justify-between px-5 py-2.5 border-t border-white/5 bg-white/[0.015]">
          <span className="text-xs text-slate-600">
            {charCount > 0 ? (
              <>
                <span className={`font-medium ${wordCount > 50 ? 'text-emerald-500' : 'text-amber-500'}`}>
                  {wordCount}
                </span>{' '}
                words
              </>
          ...
          {wordCount > 0 && wordCount < 50 && (
            <span className="text-xs text-amber-500/80">
              Tip: more detail = better analysis
            </span>
          )}
