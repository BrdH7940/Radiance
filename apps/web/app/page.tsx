...
export default function UploadPage() {
  ...
  const { cvFile, jdText, phase, setPhase, setLoadingStepIndex, setLatexCode, setPdfUrl } =
    useCVStore();
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    if (jdText.trim().length < 10) {
      setValidationError('Please paste a job description (at least a few words).');
      return;
    }
    ...
    try {
      const result = await uploadAndAnalyze(cvFile, jdText, (stepIndex) => {
  ...
  }, [cvFile, jdText, router, setLatexCode, setLoadingStepIndex, setPhase, setPdfUrl]);
        ...
        <main className="relative z-10 max-w-screen-xl mx-auto px-8 pt-12 pb-20">
          ...
          <div className="mb-14 text-center animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <p className="text-slate-400 text-lg max-w-xl mx-auto leading-relaxed">
              Upload your CV and the job description. Radiance analyzes the gap and
              regenerates your resume as publication-ready LaTeX.
            </p>
          </div>
          ...
          {/* Split-screen input */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-10 animate-in fade-in slide-in-from-bottom-8 duration-700" style={{ animationDelay: '150ms' }}>
            {/* Left — CV Upload */}
          ...
          {/* CTA Button */}
          <div className="flex justify-center animate-in fade-in slide-in-from-bottom-8 duration-700" style={{ animationDelay: '300ms' }}>
            <button
